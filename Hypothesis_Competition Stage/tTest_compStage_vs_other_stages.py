...
for index in range(len(ko_stages)):
    ...
    isGroupStage = bool(ko_stages[:index])
    if not isGroupStage:
        current_stages_df = dfShotEval.loc[dfShotEval['competition_stage'] == "Group Stage"]
    else:
        current_stages_df = dfShotEval.loc[dfShotEval['competition_stage'].isin(["Group Stage"] + ko_stages[:index])]
...
dfCompStage.to_json("tTest_competition_stage_vs_other_stages_results.json")
